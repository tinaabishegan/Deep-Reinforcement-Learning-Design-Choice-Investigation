import torch 
import torch.nn.functional as F

def init_weights_normal(m):
    if isinstance(m, torch.nn.Linear):
        torch.nn.init.normal_(m.weight, mean=0.0, std=0.1)
        torch.nn.init.constant_(m.bias, 0)

hyperparameters = {
    'CartPole-v1': {
        'BATCH_SIZE': 64,
        'LEARNING_RATE': 0.0005,
        'GAMMA': 0.99,
        'EPSILON_START': 1.0,
        'EPSILON_END': 0.01,
        'EPSILON_DECAY': 0.995,
        'MEMORY_CAPACITY': 2000,
        'TARGET_UPDATE_EVERY': 100,  # Hard target update interval
        'NUM_EPISODES': 2000,
        'MAX_T': 5000,
        'SOLVED_SCORE': 475,
        'SOLVED_LENGTH': 100,
        'HIDDEN_LAYERS': [128, 64],  # Specific to CartPole
        'ACTIVATION_FN': F.relu,
        'INIT_FN': init_weights_normal,
        'USE_NOISY': True,  # Enable noisy networks
        'NOISE_SCALE': 0.4,  # Initial noise scale
        'K_FINAL': 3.0,         # Final value of k
        'INF_REWARD': 0,     # Minimum possible reward
        'SUP_REWARD': 550,  
        'EPISODE_UPDATE_REWARD': 200,
        'UPDATED_SUP_REWARD': 575,
        'SUB_GOAL_KEYS': ['theta', 'x'],  # Keys expected in sub_goal
    },
    'LunarLander-v3': {
        'BATCH_SIZE': 128,                  # Smaller batch size for more frequent updates
        'LEARNING_RATE': 1e-4,             # Lower learning rate for stability
        'GAMMA': 0.995,
        'EPSILON_START': 1.0,
        'EPSILON_END': 0.01,
        'EPSILON_DECAY': 0.998,
        'MEMORY_CAPACITY': int(1e5),
        'TARGET_UPDATE_EVERY': 4,          # Soft target updates
        'NUM_EPISODES': 1000,
        'MAX_T': 1000,
        'SOLVED_SCORE': 200,
        'SOLVED_LENGTH': 100,
        'HIDDEN_LAYERS': [256, 128],
        'ACTIVATION_FN': F.relu,
        'INIT_FN': None,
        'TAU': 1e-3,
        # Noisy Network parameters
        'USE_NOISY': True,
        'SIGMA_INIT': 0.5,                 # Increased initial noise for better exploration
        'K_FINAL': 2.0,                    # Lower K_FINAL to reduce overemphasis on noise reduction
        'INF_REWARD': -200,                # Minimum possible reward
        'SUP_REWARD': 275,                 # Adjusted to reflect typical reward range
        'EPISODE_UPDATE_REWARD': 300,
        'UPDATED_SUP_REWARD': 300,
        'SUB_GOAL_KEYS': ['y', 'y_dot', 'x', 'x_dot', 'theta', 'theta_dot'],
    }

}
